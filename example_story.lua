-- Only register the story for the map we want it on (or on global state when we're in compatibility mode)
if (GetStateMapId() == 1 or IsCompatibilityMode()) then
    ...
    SetFromScript: $setFromScript]],  -- These are all available placeholders except setFromScript, it's an example of a story variable
            options = {
                {
                    text = [[Option 1]], -- Text of that gossip option
                    next = 2, -- Next story chapter
                ...
                },
                {
                    ...
                    next = 4,
                    onSelect = function(story, player, creature) -- You can execute code on gossip selection
                ...
                },
                {
                    ...
                    condition = function(story, player, creature) -- A condition that needs to be met to make this option available
